...
> **Widened by shopfloor#46, and it now carries the spend gate too.** Two
> things changed. First, a third fence: `head_repository.full_name` must equal
> `repository.full_name`. A `workflow_run` from a fork PR carries the fork's
> ref with the base repository in `repository`, so both the branch name and the
> commit author — a git identity anyone can set locally — are attacker-chosen
> there. Absent refuses like mismatched does.
>
> Second, on the machine edge these fences **replace** the permission probe
> rather than sitting in front of it. There is no login to probe:
> `workflow_run.triggering_actor` is frequently `github-actions[bot]`, whose
> collaborator permission is `none`, so §3's "the only edge with no human on
> it" refused every time. What authorizes a continuation is that pushing to
> `agent/issue-<n>` **in the repository itself** already requires write access,
> which is a spending permission — the fork fence is what keeps that sentence
> true. See `AdmissionAuthority` in `src/trigger/admission.ts`.
